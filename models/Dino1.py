...
class Dino(nn.Module):
    def __init__(self, num_labels, model_cfg):
        ...
        self.classifier = nn.Sequential(
            nn.Conv2d(self.backbone.config.hidden_size, self.backbone.config.hidden_size, kernel_size=3, padding=1),
            nn.GroupNorm(32, self.backbone.config.hidden_size),
            nn.ReLU(inplace=True),
            nn.Dropout2d(0.1),
            nn.Conv2d(self.backbone.config.hidden_size, num_labels, kernel_size=1),
        )
